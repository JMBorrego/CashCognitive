...
def rule_to_feature(dataset, text_rule=''):
    ...
    for string_var in string_list:
        if i == 0 and len(position_and_list) > 0:
            ...
            rule = '((' + dataset_variable_name + "['" + feature_list[i] + "']" + ' ' + text_rule[
                                                                                        all_eq_positions[0]:] + '))'
...
def rules_feature_engineering(data_X,
    ...
    for i in range(0, n_best_rules):
        ...
        text_rule_cleaned = text_rule.replace(' ', '_').replace(' ', '_').replace(' ', '_').replace('&', 'AND').replace(
            '.', '_')
        ...
        df_temp = pd.DataFrame([['RULE_EXTRACT_' + str(i + 1), selected_rules[i], statement]],
...
def feature_selection(X_train, y_train):
    ...
    for col in y_train.columns:
        ...
        df_fscore_xgb_list = pd.DataFrame(xgb_model.get_booster().get_score(importance_type='gain'),
                                          index=[0]).T.sort_values(by=(0), ascending=False)
    ...
    df_fscore_xgb = df_fscore_xgb.loc[
        df_fscore_xgb['FSCORE_GAIN_XGB_ABS'] >= df_fscore_xgb['FSCORE_GAIN_XGB_ABS'].quantile(0.45)]
    ...
    perm_df_score = pd.DataFrame(perm_df_score.groupby(['FEATURE']).mean().reset_index()).sort_values(by=['PERM_SCORE'],
                                                                                                      ascending=False)
    ...
    all_features_scaled = pd.DataFrame(scaler.fit_transform(all_features.iloc[:, 1:]),
                                       columns=all_features.iloc[:, 1:].columns, index=all_features.index)
    ...
    all_features_mean = all_features_mean.loc[
        all_features_mean['SCORE_MEAN'] >= all_features_mean['SCORE_MEAN'].quantile(0.2)]
...
class DataPreparator:
    ...
    def categorical_encoding(self, X_train, y_train,
        ...
        if strategy == 'dummy':
            ...
            print(
                "ERROR: Encoding strategy does not exists. Please select one of ['None','dummy', 'catboost', 'target']")
            sys.exit()
    ...
    def drop_outliers(self, X_train, y_train,
        ...
        if strategy == 'IQR':
            ...
            print(
                "ERROR: Outlier strategy does not exists. Please select one of ['None', 'STD', 'IQR', 'IsolationForest', 'OneClassSVM', 'KNN']")
            sys.exit()
...
class FeatureSelectorRegressor:
    ...
    def fit_transform(self, X_train, y_train):
        ...
        if self.selection_strategy_ == 'lasso':
            ...
            for col in y_train.columns:
                ...
                df_fscore_xgb_list = pd.DataFrame(xgb_model.get_booster().get_score(importance_type='gain'),
                                                  index=[0]).T.sort_values(by=(0), ascending=False)
                df_fscore_xgb_list.columns = ['FSCORE_GAIN_XGB']
                df_fscore_xgb_list = pd.merge(df_allfeatures, df_fscore_xgb_list, how='left', left_index=True,
                                              right_index=True)
                df_fscore_xgb_list = df_fscore_xgb_list.fillna(0)
                df_fscore_xgb_list = df_fscore_xgb_list.sort_values(by=(['FSCORE_GAIN_XGB']), ascending=False)
                df_fscore_xgb_list['Y_TRUE'] = col
                ...
                if i == 0:
                    ...
                    global_shap_values_train.columns = ['SHAP_MEAN']
                    global_shap_values_train['index'] = global_shap_values_train.index.astype(int)
                    global_shap_values_train = global_shap_values_train.merge(features_train, left_on='index',
                                                                              right_on='index')
                    del global_shap_values_train['index']
                    global_shap_values_train['Y_VALUE'] = col
            ...
            df_shap_mean = df_shap_mean.loc[
                df_shap_mean['SHAP_MEAN_ABS'] >= df_shap_mean['SHAP_MEAN_ABS'].quantile(self.quantile_cutoff_)]

            df_fscore_xgb = pd.DataFrame(df_fscore_xgb2.sort_values(by=['FSCORE_GAIN_XGB_ABS'], ascending=False))
            df_fscore_xgb_copy = df_fscore_xgb.copy()
            df_fscore_xgb = df_fscore_xgb.loc[
                df_fscore_xgb['FSCORE_GAIN_XGB_ABS'] >= df_fscore_xgb['FSCORE_GAIN_XGB_ABS'].quantile(
                    self.quantile_cutoff_)]
            ...
            for i in range(0, len(y_train.columns)):
                ...
                result = permutation_importance(model, X_train_scaled.iloc[:, selected_features], y_train.iloc[:, i],
                                                n_repeats=8,
                                                random_state=0)
                perm_sorted_idx = result.importances_mean.argsort()
                perm_df = pd.DataFrame(result.importances[perm_sorted_idx])
            ...
            perm_df_score = pd.DataFrame(perm_df_score.groupby(['FEATURE']).mean().reset_index()).sort_values(
                by=['PERM_SCORE'], ascending=False)
            perm_df_score_copy = perm_df_score.copy()
            perm_df_score = perm_df_score.loc[
                perm_df_score['PERM_SCORE'] >= perm_df_score['PERM_SCORE'].quantile(self.quantile_cutoff_)]
            ...
            all_features_scaled = pd.DataFrame(scaler2.fit_transform(all_features.iloc[:, 1:]),
                                               columns=all_features.iloc[:, 1:].columns, index=all_features.index)
            all_features_scaled['FEATURE'] = all_features['FEATURE']
            all_features_mean = pd.DataFrame(np.mean(all_features_scaled, axis=1))
            all_features_mean.columns = ['SCORE_MEAN']
            ...
            if len(all_features_mean) == 0:
                all_features_mean = all_features_mean_copy.sort_values(by=['SCORE_MEAN'], ascending=False).iloc[0, :][
                    'FEATURE']
        ...
        if self.selection_strategy_ in list(['severe']):
            ...
            print(
                "ERROR: Feature selection strategy does not exists. Please select one of ['shap','light', 'severe', 'permutation']")
            sys.exit()
